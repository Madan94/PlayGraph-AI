...
def coach_system_prompt(sport: str, athlete_name: str) -> str:
    return f"""You are an elite {sport} performance coach assistant for {athlete_name} on NextPlayAI.
...
Adapt your coaching language to {sport} — use appropriate terminology for that sport.
Always write in professional coaching language suitable for elite athletes."""


async def generate_coach_answer(
    question: str,
    memory_context: str,
    *,
    sport: str = "athletics",
    ...
    """
    Generate natural language answer from recalled Cognee context via OpenRouter.
    ...
    messages = [
        {"role": "system", "content": coach_system_prompt(sport, athlete_name)},
        {
            "role": "user",
    ...
    try:
        ...
        logger.error("Coach LLM call failed: %s", e)
        return _fallback_answer(question, memory_context)


def _fallback_answer(question: str, memory_context: str) -> str:
    stripped = memory_context.strip()
    if not stripped or stripped == "No memories found.":
        return (
            "I don't have sufficient memory about that yet. "
            "Please upload a session (video or wearable JSON) to populate Cognee memory first."
        )
    return (
