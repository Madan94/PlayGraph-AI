...
function NodeBox({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border bg-white/[0.04] backdrop-blur-xl min-w-[140px] shadow-lg transition-all",
        active && "border-cyan/50 shadow-cyan/20 shadow-lg",
        highlight && "border-purple/60 shadow-purple/30 shadow-xl scale-105",
        !active && !highlight && "border-white/10",
        className
      )}
    >
      ...
      <div className="px-3 py-2 border-b border-white/5">
        ...
        {subtitle && <p className="text-[9px] text-foreground/40">{subtitle}</p>}
      </div>
    ...
    <NodeBox title="Input Sources" subtitle="Multi-modal">
      <div className="space-y-1 text-[8px] text-foreground/50">
        {["Video", "Audio", "Wearables", "JSON", "Notes"].map((l) => (
          <div key={l} className="px-1.5 py-0.5 rounded bg-white/5">{l}</div>
        ))}
      </div>
    </NodeBox>
  );
...
export const GatewayNode = memo(function GatewayNode({ data }: NodeProps) {
  return (
    <NodeBox title="API Gateway" subtitle="JWT · RBAC">
      <div className="text-[8px] text-foreground/50 space-y-0.5">
...
export const IngestionNode = memo(function IngestionNode({ data }: NodeProps) {
  return (
    <NodeBox title="Ingestion" subtitle="Validate → Publish">
      <div className="text-[8px] text-foreground/50">Detect type · Metadata · Kafka</div>
    </NodeBox>
  );
});
...
export const CogneeNode = memo(function CogneeNode({ data }: NodeProps) {
  return (
    <div className="relative">
      ...
      <div className="rounded-2xl border-2 border-purple/50 bg-gradient-to-br from-purple/20 to-cyan/10 p-4 min-w-[200px] shadow-glow">
        <p className="text-sm font-bold gradient-text mb-2">Cognee Brain</p>
        <div className="grid grid-cols-2 gap-1.5">
          {["remember()", "recall()", "improve()", "forget()"].map((fn) => (
            <code key={fn} className="text-[8px] font-mono px-1.5 py-1 rounded bg-white/5 text-center">{fn}</code>
          ))}
        </div>
      </div>
    </div>
      ...
      <NodeBox title="Outputs" subtitle="Dashboard · Reports">
        <div className="flex gap-1 text-[8px]">
          {["Timeline", "Charts", "PDF"].map((o) => (
            <span key={o} className="px-1.5 py-0.5 rounded bg-white/5">{o}</span>
          ))}
        </div>
      </NodeBox>
    </div>
...
export const initialEdges = [
  { id: "e1", source: "input", target: "streaming", animated: true, style: { stroke: "#7C3AED" } },
  { id: "e2", source: "streaming", target: "gateway", animated: true, style: { stroke: "#06B6D4" } },
  { id: "e3", source: "gateway", target: "ingestion", animated: true },
  { id: "e4", source: "ingestion", target: "kafka", animated: true, style: { stroke: "#06B6D4" } },
  { id: "e5", source: "kafka", target: "workers", animated: true },
  { id: "e6", source: "workers", target: "cognee", animated: true, style: { stroke: "#7C3AED" } },
