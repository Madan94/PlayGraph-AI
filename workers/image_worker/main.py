"""Image worker — vision analysis of still images → backend memory ingest."""
...
from workers.shared.vision_llm import analyze_image

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)


async def process_image(session_id: str, asset_id: str, minio_key: str, context: dict) -> None:
    ...
    image_bytes = await asyncio.to_thread(download_bytes_from_minio, minio_key)
    mime = context.get("mime_type") or "image/jpeg"
    ...
    metrics = {"_observations": analysis.get("observations", [])}
    ...
    logger.info("Image worker → ingest for session %s", session_id)
...
async def main() -> None:
    ...
    consumer = AIOKafkaConsumer(
        ...
        group_id="image-worker",
        value_deserializer=lambda m: json.loads(m.decode()),
        auto_offset_reset="earliest",
    )
    await consumer.start()
    logger.info("Image worker listening on image.process.requested")
    ...
    try:
        async for msg in consumer:
            ...
            try:
                await process_image(session_id, asset_id, minio_key, p)
